...
from app.schemas.auth_schema import LoginRequest, OAuthLoginRequest, RegisterRequest, TokenResponse
...
def oauth_yandex(payload: OAuthLoginRequest, service: AuthService = Depends(get_auth_service)) -> TokenResponse:
...
def oauth_vk(payload: OAuthLoginRequest, service: AuthService = Depends(get_auth_service)) -> TokenResponse:
    return service.oauth_login(
        request=OAuthRequest(
            provider="vk",
            code=payload.code,
