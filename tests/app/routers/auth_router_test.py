...
class StubOAuthGateway(OAuthGateway):
    def fetch_user_info(self, provider: str, code: str, redirect_uri: str) -> OAuthUserInfo:
        return OAuthUserInfo(provider=provider, subject="stub", email=f"{provider}@example.com", phone=None)
...
def test_auth_router_handlers(db_session):
    ...
    yandex_response = oauth_yandex(payload=OAuthLoginRequest(code="code", redirect_uri="https://cb"), service=service)
    assert yandex_response.user.email == "yandex@example.com"

    vk_response = oauth_vk(payload=OAuthLoginRequest(code="code", redirect_uri="https://cb"), service=service)
